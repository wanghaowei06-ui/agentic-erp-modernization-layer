...
- [ ] Add browser automation for PO-1001 at `http://localhost:8000/purchase-orders/PO-1001`.
- [ ] Extract fields from stable HTML IDs on the mock ERP detail page.
- [ ] Build the triage JSON body from extracted fields.
- [ ] Call the triage service with `POST http://localhost:8001/triage`.
...
- [ ] Call the validation service with `POST http://localhost:8003/validate/request-purchase-order-approval`.
- [ ] Parse `validation_result_json` and set `validation_status`.
- [ ] Add a trusted tool registration approval placeholder or task.
- [ ] Call the API facade with `POST http://localhost:8002/api/purchase-orders/PO-1001/approval-request`.
